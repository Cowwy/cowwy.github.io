...
function updatePooStore( ) {
    ...
    for( key in PooClickerData.upgrade ) {
        if( $ST.isUpgradeUnLock( key ) ) {
            ...
            let sum = PooClickerData.calcPrice( key, $ST.getLevel( key ) );
            ...
            let level = document.createElement( "span" );
                ...
                level.innerHTML = $ST.getLevel( key );
            ...
            function upgradeLevel( e ) {
                ...
                if( $ST.getBuyOrSell( ) ) {
                    if( $ST.isUpgradePurchaseable( id ) ) {
                        ...
                        $ST.subtractPoo( PooClickerData.calcSumPrice( id, $ST.getLevel( id ), $ST.getBuyOrSellQuantity( ) ) );
                        ...
                        level.innerHTML = $ST.getLevel( id );

                        //Recalculate Cost
                        let cost  = $D.id( id + "Cost" );	
                        let sum   = PooClickerData.calcSumPrice( id, $ST.getLevel( id ), $ST.getBuyOrSellQuantity( ) );			
                        cost.innerHTML = $ST.getDisplayNotation( sum );
                ...
                //=============================================
                    ...
                    let pooStats = $D.id( "totalPooCollected" );
                    $ST.addPoo( PooClickerData.calcSellPrice( id, $ST.getLevel( id ), $ST.getBuyOrSellQuantity( ) ) );
                    ...
                    //SET LEVEL TO 0
                    if( $ST.getBuyOrSellQuantity( ) > $ST.getLevel( id ) ) {
                        $ST.upgradeLevelDown( id, $ST.getLevel( id ) );
                    }
                    ...
                    //===============================
                    let cost  = $D.id( id + "Cost" );	
                    cost.innerHTML = PooClickerData.calcSellPrice( id, $ST.getLevel( id ), $ST.getBuyOrSellQuantity( ) );
	...
	//===================================================
		...
		temp.forEach( ( key ) => {
			...
			if( $D.id( key + "Upgrade" ) ) {
				...
				el.className = $ST.getLevel( key ) === 0 ? "cost notPurchaseable" : "cost purchaseable";				
			}
		});
	}
}
